...
/**
 * Optimized matrix transposition methods for the <code>rawtype</code> type.
...
public class RawtypeMatrixStrategy
    implements MatrixStrategy
{
    ...
    public void transposeSquare(ArrayAccess arrayAccess, int n1, int n2)
        throws ApfloatRuntimeException
    {
        transposeSquare(arrayAccess.getRawtypeData(), arrayAccess.getOffset(), n1, n2);
    }
    ...
    private static void moveBlock(rawtype[] source, int sourceOffset, int sourceWidth, rawtype[] dest, int destOffset, int destWidth, int b)
    {
        for (int i = 0; i < b; i++)
        {
    ...
    private static void transpose2blocks(rawtype[] data, int offset1, int offset2, int width, int b)
    {
        for (int i = 0, position1 = offset2; i < b; i++, position1 += width)
        {
            for (int j = 0, position2 = offset1 + i; j < b; j++, position2 += width)
            {
                rawtype tmp = data[position1 + j];
    ...
    private static void transposeBlock(rawtype[] data, int offset, int width, int b)
    {
    ...
    private static void transposeSquare(rawtype[] data, int offset, int n1, int n2)
    {
        ...
        else
        {
            ...
            rawtype[] tmp1 = new rawtype[b * b],
                      tmp2 = new rawtype[b * b];
    ...
    private static void permuteToHalfWidth(rawtype[] data, int offset, int n1, int n2)
    {
        if (n1 < 2)
        {
            return;
        ...
        int twicen1 = 2 * n1;
        int halfn2 = n2 / 2;
        rawtype[] tmp = new rawtype[halfn2];
    ...
    private static void permuteToDoubleWidth(rawtype[] data, int offset, int n1, int n2)
    {
        if (n1 < 4)
        {
            return;
